...
if (place_meeting(x + hspeed, y + vspeed, objBlock)) {
    if (vspeed > 0) {
        move_contact(objBlock, 270, abs(vspeed) + 1, 1)
        vspeed = 0
    }
}
...
if (place_meeting(x + hspeed, y + vspeed, objBlock)) {
    if (vspeed < 0) {
        move_contact(objBlock, 90, abs(vspeed), 1)
        move_outside(objBlock, 270, abs(vspeed) + 1, 1)
        vspeed = 0
    }
}
...
if (place_meeting(x, y, objBlock)) {
    move_outside(objBlock, 90, abs(vspeed) + 3)
    move_outside(objBlock, 270, abs(vspeed) + 3)
    move_outside(objBlock, 0, maxHSpeed)
    move_outside(objBlock, 180, maxHSpeed)
}